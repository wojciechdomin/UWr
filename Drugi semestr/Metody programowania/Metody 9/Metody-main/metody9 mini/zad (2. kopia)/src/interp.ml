...
let rec show_value v =
  ...
  | VRef _ -> "<ref>"
  | _ -> failwith "not implemented! show_value"
...
let rec eval_env (env : env) (e : expr) (h : heap) : value * heap =
  match H.find 0 h with 
   | VBreak _ -> (VUnit,h)
   | VContinue _ -> (VUnit,h) (* mogloby byc tu cokolwiek, bo i tak przywrocimy stan heapa w while'u...
to jest tylko po to by terminowac wykonywanie sie instrukcji*)
   | VUnit -> (
  match e with
  | Int i -> (VInt i, h)
  | Bool b -> (VBool b, h)
  ...
  | Break -> (match H.find 0 h with
	      |	VBreak _ -> (VUnit, h)
	      | VContinue _ -> (VUnit, h)
              | VUnit -> (VUnit, H.add 0 (VBreak h) h)
   	      | _ -> failwith "break exception"
	      )
 | Continue -> (match H.find 0 h with
	      |	VBreak _ -> (VUnit, h)
	      | VContinue _ -> (VUnit, h)
              | VUnit -> (VUnit, H.add 0 (VContinue h) h)
   	      | _ -> failwith "continue exception"
	      ) 
	
  | While (cond,e) -> 
	let (v,h) = eval_env env cond h in
		(match v with
			| VBool true -> let (_,h) = eval_env env e h
				in 
				let status = H.find 0 h in 
				(match status with
				| VUnit -> eval_env env (While (cond,e)) h
				| VBreak h -> (VUnit, h)
				| VContinue h ->  eval_env env (While (cond,e)) h
				| _ -> failwith "!"
				)
			| VBool false -> (VUnit, h)  
			| _ -> failwith "warunek powinien zwrocic wartosc logiczna!!11!1!"
		) 
	)
   | _ -> failwith "!"	
	

let eval e = fst (eval_env M.empty e (H.empty |> H.add 0 VUnit))
