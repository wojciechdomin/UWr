...
let rec eval (e : expr) : value =
  ...
  | Sum (x,e1,e2,e3) -> (match (eval e1, eval e2) with 
         | (VInt n, VInt m) -> let rec sum cnt = if n > m then 0 else let res = (match eval(subst x (Int cnt) e3) with | VInt res -> res | _ -> failwith "!" ) in if cnt < m then res + sum (cnt+1) else res in  VInt (sum n)
         | _ -> failwith "!!"
	)
