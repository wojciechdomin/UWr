...
let rec subst (x : ident) (s : expr) (e : expr) : expr =
  ...
  | Match (p, v1, v2, e) -> Match (subst x s p, v1, v2, if x = v1 || x = v2 then e else subst x s e)
  | _ -> e
...
let rec eval (e : expr) : value =
  ...
  | Match (p, x, y, e) -> (match reify (eval p) with | Pair (e1,e2) -> eval (subst x e1 (subst y e2 e)) | _ -> failwith "wrong match!" )
