...
let rec subst (x : ident) (s : expr) (e : expr) : expr =
  ...
  | Check (c1, e1) -> Check (c1, subst x s e1)
  | Fold (e1,y,acc,e2,e3) -> Fold (subst x s e1, y, acc, (if y = x || acc = x then e2 else subst x s e2), subst x s e3)
  | _ -> e
...
let rec eval_fold v1 x acc e2 e3 =
( match v1 with | VPair(v1,v2) -> eval ( Let(acc,reify (eval_fold v2 x acc e2 e3), Let(x, reify v1, e2) )  ) | VUnit -> eval(e3)       | _ -> failwith "!" )
 and 
...
eval (e : expr) : value =
  ...
  | Check (Boo, e1) -> (match (eval e1) with | VBool _ -> VBool true | _ -> VBool false)
  | Check (Num, e1) -> (match (eval e1) with | VInt _ -> VBool true | _ -> VBool false)
  | Check (Pai, e1) -> (match (eval e1) with | VPair _ -> VBool true | _ -> VBool false)
  | Check (Uni, e1) -> (match (eval e1) with | VUnit -> VBool true | _ -> VBool false)
  | Fold (e1, x, acc, e2, e3) -> ( match (eval e1) with | VPair(v1,v2) -> eval ( Let(acc,reify (eval_fold v2 x acc e2 e3), Let(x, reify v1, e2) )  ) | VUnit -> eval(e3)       | _ -> failwith "!" )
